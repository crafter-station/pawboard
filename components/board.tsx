...
export function Board({
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Cmd+/ or Ctrl+/ to toggle chat panel (desktop only)
  ...
  // Get the first selected card ID for chat context
  const selectedCardIdForChat =
    selectedCardIds.size === 1
      ? selectedCardIds.values().next().value
      : undefined;
